...
__global__ void dotpdt(float* a, float* b, float* c) 
{
...
int main (void) {
    ...
    dotpdt<<<blocksPerGrid, threadsPerBlock>>>(dev_a, dev_b, dev_partial_c);
    ...
    printf("CPU sequential value = %.6g, In time = %.6f ms\n",2*sum_squares((float)(N-1)),t);
    printf("GPU parallelised value = %.6g, In time = %.6f ms\n",c,milliseconds);
